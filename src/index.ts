...
// 長い絵文字（ZWJ 結合・バリエーションセレクタ付き）を先に置換するため、文字数降順でソート
const sortedEmojiEntries = Object.entries(emojiJapaneseNames).sort((a, b) => b[0].length - a[0].length);

// 未登録の絵文字を捕捉する正規表現（ZWJ 結合・スキントーン・バリエーションセレクタを含む）
...
function replaceUnicodeEmojiWithJapanese(text: string): string {
  ...
  // スキントーン修飾子が残った場合は除去（例: 👍🏽 → 👍 → いいね）
  output = output.replace(/([\u{1F3FB}-\u{1F3FF}])/gu, "");
  return output;
}

function replaceUnknownEmojiWithFallback(text: string): string {
  return text.replace(unknownEmojiPattern, (match) => {
    // 既に日本語名に置換済み（前後に空白が付いた状態）の場合はそのまま
...
async function applyEmojiAndDictionaryTransform(guildId: string, text: string): Promise<string> {
  ...
  return applyLaughNormalization(withFallbackEmoji).replace(/\s+/g, " ").trim();
}
